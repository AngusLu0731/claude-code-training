...
起手式：第 4 章貼的 `spec_gate.py` 已經被步驟 1 的重置洗掉了，但練習 repo 的 `solutions/` 留著一份——`cp solutions/ch4-hooks/.claude/hooks/spec_gate.py .claude/hooks/push_gate.py`。`gate()` 的第 1、2 段（拿 `file_path`、算相對路徑）都不需要了，改成從 `tool_input` 拿 `command`，只留第 3 段的規則判斷。規則本身不用正則，`"git" in command and "push" in command` 這種最簡單的寫法就能過驗收（代價是 `git commit -m "push fix"` 也會被擋——規格說了寧可誤擋）。
...
正文步驟（順序就是安全網）：版本號從 `$ARGUMENTS` 拿，沒給就停下來問；先確認在 `main` 且工作樹乾淨（`git status --porcelain` 沒有輸出——版本要建立在全部 commit 的狀態上），不是就停下來回報；再跑 `python3 -m unittest -v`，最後不是 `OK` 就停；用 `git describe --tags --abbrev=0`（從目前的 commit 往回找最近的一個 tag）找上一個 tag，有就只看 `<tag>..HEAD` 的 `git log --oneline`、沒有就看全部，依 type 分組（第 1 章 changelog 的格式）；把 `## <version> (YYYY-MM-DD)` 那段插到 `CHANGELOG.md` 最上面（沒有就建、有就插在第一行前）；`git add CHANGELOG.md && git commit -m "chore: release <version>"`；`git tag v<version>`；最後印結果並提醒**沒有 push**。限制：只動 `CHANGELOG.md`；絕不 push。
...
8. **收斂（主 checkout）**：
   ...
   預期 `Ran 8 tests`（或更多——SPEC.md 只要求至少加兩個測試，Claude 多寫幾個也算）與 `OK`——`tests.test_todo` 只跑 `tests/test_todo.py` 這一個檔（`main` 上 6 個，feat 至少加 2 個）；第 5 章的 `python3 -m unittest -v` 跑的是全部三個檔的 31 個。
