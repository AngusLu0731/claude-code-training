...
起手式：把第 4 章的 `spec_gate.py` 複製成 `push_gate.py`。`gate()` 的第 1、2 段（拿 `file_path`、算相對路徑）都不需要了，改成從 `tool_input` 拿 `command`，只留第 3 段的規則判斷。規則本身不用正則，`"git" in command and "push" in command` 這種最簡單的寫法就能過驗收（代價是 `git commit -m "push fix"` 也會被擋——規格說了寧可誤擋）。
...
正文步驟（順序就是安全網）：版本號從 `$ARGUMENTS` 拿，沒給就停下來問；先確認在 `main` 且工作樹乾淨（`git status --porcelain` 沒有輸出——版本要建立在全部 commit 的狀態上），不是就停下來回報；再跑 `python3 -m unittest -v`，最後不是 `OK` 就停；用 `git describe --tags --abbrev=0` 找上一個 tag，有就只看 `<tag>..HEAD` 的 `git log --oneline`、沒有就看全部，依 type 分組（第 1 章 changelog 的格式）；把 `## <version> (YYYY-MM-DD)` 那段插到 `CHANGELOG.md` 最上面（沒有就建、有就插在第一行前）；`git add CHANGELOG.md && git commit -m "chore: release <version>"`；`git tag v<version>`；最後印結果並提醒**沒有 push**。限制：只動 `CHANGELOG.md`；絕不 push。
...
| **Verdict**：一個 agent | 上一站合併後的 findings JSON | schema：`verdict` 只能是 `go` 或 `no-go`（用 `enum`，第 3 章那張「六個寫法」表的最後一列），加 `reasons` 字串陣列 | `return` 它，對話裡就看得到 |

規格：`meta.name` 是 `'ship-check'`、有 `description`、`phases` 列出兩站；prompt 要**自足**（subagent 看不到主對話，要讀哪些檔、對照什麼、回什麼都寫進去）；每個 agent 都不改檔。

做法：把第 3 章的 `review-flow.js` 複製成 `ship-check.js`，改五處——① `meta` 的 name／description／phases；② `reviewPrompt` 改成「先讀 SPEC.md 再對照指定檔案」的 prompt；③ `parallel` 裡兩個 agent 的檔案與 label；④ 加一個 `VERDICT` schema，最後那個 `agent()` 的選項加 `schema: VERDICT`；⑤ `return summary` 改成 `return verdict`。Verdict 的 schema 長這樣：
...
3. **貼 SPEC.md、寫四樣工具**：照上面「規格」的 0 到 4 節，在終端機貼 `SPEC.md`，然後寫 `settings.json`＋`push_gate.py`、`release/SKILL.md`、`doc-checker.md`、`ship-check.js`。`release` 是唯一要自己建目錄的：先 `mkdir -p .claude/skills/release`（其他三個資料夾練習 repo 已預建，直接 `cat >` 或用編輯器存檔）。每寫完一份就做那一節的自測；hook 與 workflow 的自測不用開 `claude`，現在就能做。
...
8. **收斂（主 checkout）**：
   ...
   預期 `Ran 8 tests` 與 `OK`——`tests.test_todo` 只跑 `tests/test_todo.py` 這一個檔（`main` 上 6 個，feat 加了 2 個）；第 5 章的 `python3 -m unittest -v` 跑的是全部三個檔的 31 個。
...
- **驗收說 merge 節點不夠**：收斂時少了 `--no-ff`，git 用 fast-forward 把 `main` 直接挪到分支尾端、沒留合併 commit。重來：`git log --oneline` 找到訊息是「chore: release 工具鏈…」那行前面的 7 碼 hash → `git tag -d v1.0.0` → `git reset --hard <那個 hash>` → 重做步驟 8（這次記得 `--no-ff --no-edit`；分支還在就不用重做 worktree）→ 重做步驟 10 的 `/release`。
